...
void main() {
  ...
  group('RideSessionRepositoryImpl', () {
    ...
    test(
      ...
      () async {
        dataSource.errorToThrow = const PostgrestException(
          message: 'this group already has an active ride session',
          code: 'P0001',
        );
    ...
    test('translates the RPC "already <status>" message (with interpolated status) to Spanish', () async {
      dataSource.errorToThrow = const PostgrestException(
        message: 'this ride session is already finished',
        code: 'P0001',
      );
    ...
    test(
      ...
      () async {
        dataSource.errorToThrow = const PostgrestException(
          message: 'only the group owner or an admin can start a ride session',
          code: 'P0001',
        );
